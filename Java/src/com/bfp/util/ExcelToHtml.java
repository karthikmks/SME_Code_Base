...
public class ExcelToHtml {
	private static final Log log = LogFactory.getLog(ExcelToHtml.class);
    final private StringBuilder body = new StringBuilder(65536);
    private String xslToHtmlMain = "";
    ...
    final private String BODY_CONTENTS = ":BODY-CONTENTS:";
    final private String FOOTER_CONTENTS = ":FOOTER-CONTENTS:";
     ...
     */
    public ExcelToHtml(final InputStream in,String[] sheetNamesToBeProcess) throws IOException {
    	sheetNames = sheetNamesToBeProcess;
        ...
        for (int i = 0; i < sheetNames.length; ++i) {
        	try{
        		HSSFSheet sheet = book.getSheet(sheetNames[i]);
        		if(sheet!=null){
        			log.info("Start proccessing sheet==> "+sheetNames[i]);
        			table(sheet);
        		}else{
        			log.info("Sheet not found==> "+sheetNames[i]);
        		}
        	}catch(Exception e){
        		log.error("Exception while processing sheet==> "+sheetNames[i]);
    ...
    private void table(final HSSFSheet sheet) {
        ...
        if (sheet.getDrawingPatriarch() != null) {
	        ...
	        body.append("<table cellspacing='0' style='border-spacing:0; border-collapse:collapse;'>\n");
    ...
    private void td(final HSSFCell cell) {
        ...
        case CellStyle.ALIGN_LEFT:
            body.append("text-align: left; ");
            break;
        case CellStyle.ALIGN_RIGHT:
            body.append("text-align: right; ");
            break;
        ...
        if (style.getBorderLeft() != HSSFCellStyle.BORDER_NONE) {
            body.append("border-left-style: solid; ");
        }
        body.append("'>");
        String val = "";
        try {
            ...
            case HSSFCell.CELL_TYPE_FORMULA:
                ...
                case Cell.CELL_TYPE_BOOLEAN:
                    body.append(cv.getBooleanValue());
                    break;
                case Cell.CELL_TYPE_NUMERIC:
                    body.append(cv.getNumberValue());
                    break;
                case Cell.CELL_TYPE_STRING:
                    body.append(cv.getStringValue());
                    break;
        ...
        if (pix.containsKey(rowIndex)) {
            if (pix.get(rowIndex).containsKey(colIndex)) {
                for (final HSSFPictureData pic : pix.get(rowIndex)
                        .get(colIndex)) {
                    body.append("<img alt='Image in Excel sheet' src='data:");
                    body.append(pic.getMimeType());
                    body.append(";base64,");
                    try {
    ...
    public String getHTML() {
        xslToHtmlMain = getHTMLTemplate();
        xslToHtmlMain = xslToHtmlMain.replace(BODY_CONTENTS, body).replace(FOOTER_CONTENTS, generateFooter());
        return xslToHtmlMain;
    }
    
    private String generateFooter(){
    	StringBuilder footer = new StringBuilder("");
    	footer.append("<table border=0 cellspacing=1><tr>");
    	for(int index=0;index<sheetNames.length;index++){
    		footer.append("<td bgcolor='#FFFFFF' nowrap><b><small><small>&nbsp;"
    			+"<a href='#' onclick='showHideTable("+index+")' >"
    			+"<font face='Arial' color='#000000'>"
    			+sheetNames[index]+"</font></a>&nbsp;</small></small></b></td>");
    	}
    ...
    private String getHTMLTemplate(){
    	...
    	+"function showHideTable(myIndex){"+"\n"
    	+"var tableList = document.getElementById('tableContainer').getElementsByTagName('table');"+"\n"
    	+"for( childIndex=0; childIndex< tableList.length; childIndex++ ){"+"\n"
    	+"var childTable = tableList[childIndex];"+"\n"
    	+"if(myIndex == childIndex){"+"\n"
    	...
    	+"</script>"+"\n"
    	+"</head>"+"\n"
    	+"<body onload='showHideTable(0)' topmargin=0 leftmargin=0 bgcolor='#808080'>"+"\n"
    	+"<div id='tableContainer' class='tableContainerStyle'>"+"\n\n"
    	+":BODY-CONTENTS:"+"\n\n"
    	+"</div>"+"\n\n\n"
    	+":FOOTER-CONTENTS:"+"\n"
    	+"</body>"+"\n"
